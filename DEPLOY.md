...
O servidor (`server/index.js`, via `server/lib/db.js`) escolhe o backend
sozinho, sem precisar editar código:

- **Com `SUPABASE_URL` e `SUPABASE_SERVICE_ROLE_KEY` definidos** → usa
  Postgres/Supabase (`server/lib/db.supabase.js`). **Recomendado para
  ...
  migrations em [`supabase/migrations/`](supabase/migrations/) (rode todas,
  em ordem, no SQL Editor do seu projeto).
- **Sem essas variáveis** → cai automaticamente em arquivos JSON locais
  (`server/lib/db.json.js`), gravados em `server/data/restaurants/<slug>/`
...
- **Local:** a senha padrão é `admin123` (veja `server/index.js`) — só
...
5. Se estiver usando Supabase, rode todas as migrations de
   `supabase/migrations/` (em ordem) no SQL Editor do projeto antes do
...
- **Persistent Disk** (recurso pago do Render) montado em `server/data`, se
...
(`server/data/uploads/<slug>/`), servido em `/uploads/<slug>/<arquivo>` —
...
- `SUPABASE_SERVICE_ROLE_KEY` só é lida no backend (`server/lib/`) — nunca
...
`scripts/seed-restaurants.mjs` e rode `node scripts/seed-restaurants.mjs`
