...
import { supabase, supabaseEnabled } from './lib__supabaseClient.js';
import { isCloudinaryConfigured, uploadImageBuffer } from './lib__cloudinary.js';
...
async function saveLocal(file, slug) {
  ...
  const filename = `upload-${Date.now()}-${crypto.randomBytes(8).toString('hex')}${safeExt(file.originalname)}`;
...
export async function saveImage(file, slug) {
  ...
  if (production) {
    throw new Error('Armazenamento de imagens não configurado em produção. Configure Supabase Storage ou Cloudinary.');
  }
