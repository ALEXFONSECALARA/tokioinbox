...
// em backend/index.js antes da Fase 2, só movida pra cá sem NENHUMA mudança
// de comportamento. Continua sendo o fallback automático quando o Supabase
// não está configurado (ver backend/db.js e backend/supabaseClient.js).
...
const RESTAURANTS_FILE = path.join(DATA_DIR, 'storage__legacy-json__restaurants.json');
const PLATFORM_FILE = path.join(DATA_DIR, 'storage__legacy-json__platform.json');
const ADMIN_USERS_FILE = path.join(DATA_DIR, 'storage__legacy-json__admin-users.json');
const CUSTOMERS_FILE = path.join(DATA_DIR, 'storage__legacy-json__customers.json');
const CUSTOMER_ADDRESSES_FILE = path.join(DATA_DIR, 'storage__legacy-json__customer-addresses.json');
const ADMIN_LOGIN_LOGS_FILE = path.join(DATA_DIR, 'storage__legacy-json__admin-login-logs.json');
const ERROR_LOGS_FILE = path.join(DATA_DIR, 'storage__legacy-json__error-logs.json');
...
function menuPath(slug) {
  return path.join(DATA_DIR, `storage__legacy-json__restaurants__${slug}__menu.json`);
}
function configPath(slug) {
  return path.join(DATA_DIR, `storage__legacy-json__restaurants__${slug}__config.json`);
}
function ordersPath(slug) {
  return path.join(DATA_DIR, `storage__legacy-json__restaurants__${slug}__orders.json`);
}
...
export async function createCustomer({ name, phone, email, passwordHash }) {
  ...
  if (normalized.startsWith('55') && normalized.length > 11) variants.add(normalized.slice(2));
  else if (normalized.length === 10 || normalized.length === 11) variants.add(`55${normalized}`);
  if (customers.some((c) => variants.has(String(c.phone || '').replace(/\D/g, '')))) {
...
export async function getCustomerByPhone(phone) {
  ...
  if (clean.startsWith('55') && clean.length > 11) variants.add(clean.slice(2));
  else if (clean.length === 10 || clean.length === 11) variants.add(`55${clean}`);
  return customers.find((c) => variants.has(String(c.phone || '').replace(/\D/g, ''))) || null;
}
...
export async function clearCustomerOrderHistory(customerId) {
  ...
  for (const r of restaurants) {
    const orders = await readJson(ordersPath(r.slug), []);
    const kept = orders.filter((o) => o.customerId !== customerId || !['entregue', 'cancelado'].includes(o.status));
    removed += orders.length - kept.length;
    if (kept.length !== orders.length) await writeJson(ordersPath(r.slug), kept);
  }
  return { removed };
}
