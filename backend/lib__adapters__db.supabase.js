...
import { supabase } from './lib__supabaseClient.js';
...
export async function getCustomerByPhone(phone) {
  ...
  const { data, error } = await supabase.from('customers').select('*').in('phone', [...variants]).limit(2);
  if (error) throw error;
  const row = (data || []).find((item) => String(item.phone || '').replace(/\D/g, '') === clean) || data?.[0] || null;
  return row ? customerRowToApi(row) : null;
}
