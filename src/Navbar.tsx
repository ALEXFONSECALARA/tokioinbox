      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3 min-h-[44px]">
          {currentView === 'menu' && (
            ...
            >
              {/* V7: removido o rótulo textual "Cardápio: Nome" — agora é um
                  chip com o logo/emoji do restaurante + nome, identidade
                  visual em vez de texto repetitivo. */}
              <span className="text-xs font-bold text-[#C5A880] flex items-center gap-1.5 bg-black/60 px-2.5 py-1 rounded-lg border border-[#C5A880]/30">
                {currentRestaurant?.logo ? (
                  ...
                  <span className="text-sm leading-none">{currentRestaurant?.emoji || '🍽️'}</span>
                )}
                <span>{currentRestaurant ? currentRestaurant.name : 'Cardápio'}</span>
              </span>
            </button>
          )}
        </div>
