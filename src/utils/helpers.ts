...
// Volume dos alertas sonoros (evolução v25, prioridade 4: "Volume
// configurável"). Multiplicador único aplicado dentro de beep() — afeta
// TODOS os sons (novo pedido, atraso, conexão), sem precisar mexer em cada
// som individualmente. 0 = mudo, 1 = volume máximo (comportamento de antes).
const ALERT_VOLUME_STORAGE_KEY = 'tokioinbox_alert_volume';
let alertVolume = (() => {
  try {
    const v = parseFloat(localStorage.getItem(ALERT_VOLUME_STORAGE_KEY) || '1');
...
export function setAlertVolume(v: number) {
  alertVolume = Math.min(1, Math.max(0, v));
  try {
    localStorage.setItem(ALERT_VOLUME_STORAGE_KEY, String(alertVolume));
  } catch {
    // localStorage bloqueado (modo privado) — só não persiste entre sessões
  }
}
...
function beep(ctx: AudioContext, {
  ...
  const effectiveGain = Math.max(0.0001, gain * alertVolume);
  g.gain.setValueAtTime(0.0001, start);
  g.gain.exponentialRampToValueAtTime(effectiveGain, start + 0.015);
  g.gain.setValueAtTime(effectiveGain, start + duration - 0.03);
...
// Alerta de PEDIDO ATRASADO (evolução v25, prioridade 4) — som distinto do
// de pedido novo, pra não confundir as duas situações de ouvido.
export function playDelayedOrderAlertSound() {
  try {
    const ctx = getSharedAudioContext();
    if (!ctx) return;
    const now = ctx.currentTime;
    // Dois tons graves e longos, mais "urgente e cansativo" que animado —
    // combina com "isso está demorando", diferente da sirene animada de
    // pedido novo.
    beep(ctx, { freq: 440, start: now, duration: 0.4, type: 'sawtooth', gain: 0.9 });
    beep(ctx, { freq: 349, start: now + 0.45, duration: 0.5, type: 'sawtooth', gain: 0.9 });
  } catch {
    // idem — nunca deixa um erro de áudio quebrar o resto do painel
  }
}

// Alarme CONTÍNUO de conexão caída (evolução v25, prioridade 1) — toca em
// loop enquanto o painel estiver marcado como offline, até reconectar ou o
// admin mutar. Devolve uma função pra parar o loop.
let connectionAlarmInterval: ReturnType<typeof setInterval> | null = null;
export function startConnectionAlarm() {
  if (connectionAlarmInterval) return; // já está tocando, não duplica o loop
  const tick = () => {
    try {
      const ctx = getSharedAudioContext();
      if (!ctx) return;
      const now = ctx.currentTime;
      beep(ctx, { freq: 880, start: now, duration: 0.18, type: 'square', gain: 0.8 });
      beep(ctx, { freq: 880, start: now + 0.25, duration: 0.18, type: 'square', gain: 0.8 });
    } catch {
      // idem
    }
  };
  tick();
  connectionAlarmInterval = setInterval(tick, 3500);
}
export function stopConnectionAlarm() {
  if (connectionAlarmInterval) {
    clearInterval(connectionAlarmInterval);
    connectionAlarmInterval = null;
  }
}
