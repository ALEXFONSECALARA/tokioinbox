...
import { fetchMenu, toPublicSlug, deleteDriverAdmin, uploadImage } from './utils__api';
import { LAYOUTS } from './utils__layouts';
import { ToolsHub } from './components__admin__ToolsHub';
import { ReceiptPrintModal } from './components__printing__ReceiptPrintModal';
import { createPrintJob, updatePrintJob } from './utils__api';
import { ImageUploadField } from './components__system__ImageUploadField';
...
import { NotificationsPanel } from './components__admin__NotificationsPanel';
import { AdminAiPanel } from './components__admin__AdminAiPanel';
...
interface AdminDashboardProps {
  ...
  onClearOrderHistory?: (password?: string) => Promise<boolean>;
  menuItems: MenuItem[];
  categories: Category[];
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  // Volume dos alertas sonoros (evolução v25, prioridade 4). Estado local só
  // pra reatividade do slider — quem guarda o valor de verdade é getAlertVolume/
  // setAlertVolume (persistido em localStorage, compartilhado com o resto do app).
  const [alertVolume, setAlertVolumeState] = useState(() => getAlertVolume());
  ...
  // Senha de limpeza de histórico própria deste restaurante (evolução v24_2)
  const [showClearHistoryPasswordModal, setShowClearHistoryPasswordModal] = useState(false);
  const [clearHistoryPasswordInput, setClearHistoryPasswordInput] = useState('');
  const [clearHistoryError, setClearHistoryError] = useState<string | null>(null);
  const [clearHistoryLoading, setClearHistoryLoading] = useState(false);
  // Campos de definição das senhas (Configurações → Segurança) — nomes
  // distintos dos de cima de propósito: aqueles são pra CONFIRMAR a limpeza
  // agora, estes são pra DEFINIR a senha que será exigida dali pra frente.
  const [kanbanPasswordSettingInput, setKanbanPasswordSettingInput] = useState('');
  const [historyClearPasswordSettingInput, setHistoryClearPasswordSettingInput] = useState('');

  const handleClearHistoryClick = async () => {
    const n = orders.filter((o) => ['entregue', 'cancelado'].includes(o.status)).length;
    if (!n) return alert('Não há histórico finalizado para excluir.');
    // Restaurante configurou uma senha própria pra essa ação (além da
    // permissão de conta) — pede a senha em vez do confirm() simples.
    if (localConfig.historyClearPasswordSet) {
      setClearHistoryError(null);
      setClearHistoryPasswordInput('');
      setShowClearHistoryPasswordModal(true);
      return;
    }
  ...
  const handleConfirmClearHistoryWithPassword = async () => {
    if (!clearHistoryPasswordInput.trim() || !onClearOrderHistory) return;
    setClearHistoryLoading(true);
    setClearHistoryError(null);
    try {
      await onClearOrderHistory(clearHistoryPasswordInput.trim());
      setShowClearHistoryPasswordModal(false);
      setClearHistoryPasswordInput('');
    } catch (err: any) {
      setClearHistoryError(err?.message || 'Não foi possível limpar o histórico.');
    } finally {
  ...
  const handleSaveConfig = (e: React.FormEvent) => {
    e.preventDefault();
    // Senhas próprias deste restaurante (Kanban individual / limpeza de
    // histórico) só entram no payload se o admin digitou algo no campo —
    // nunca ficam armazenadas em texto puro no estado do formulário depois
    // de salvar, e campo vazio significa "não mexer no que já está salvo".
    const payload: RestaurantConfig & { kanbanPassword?: string; historyClearPassword?: string } = { ...localConfig };
    if (kanbanPasswordSettingInput) payload.kanbanPassword = kanbanPasswordSettingInput;
    if (historyClearPasswordSettingInput) payload.historyClearPassword = historyClearPasswordSettingInput;
    onUpdateConfig(payload);
    setKanbanPasswordSettingInput('');
    setHistoryClearPasswordSettingInput('');
    setConfigSaved(true);
    playSoundEffect('success');
              ...
              <div className="absolute top-full right-0 mt-2 w-72 bg-white rounded-2xl border border-stone-200 shadow-xl p-3 z-50 text-stone-900">
                ...
                <div className="mb-3 pb-3 border-b border-stone-100">
                  <label className="flex items-center justify-between text-xs font-bold text-stone-700 mb-1">
                    ...
                    <span className="text-stone-400 font-normal">{Math.round(alertVolume * 100)}%</span>
                  </label>
                  <input
                    type="range"
                    min={0}
                    ...
                    onChange={(e) => {
                      ...
                      setAlertVolume(v);
                      setAlertVolumeState(v);
                    }}
                    onMouseUp={() => { unlockOrderAlertAudio(); playOrderAlertSound(selectedAlertSoundId); }}
                    onTouchEnd={() => { unlockOrderAlertAudio(); playOrderAlertSound(selectedAlertSoundId); }}
                    className="w-full accent-amber-500"
                  />
                  <p className="text-[10px] text-stone-400 mt-0.5">
                    Vale pra todos os sons: pedido novo, pedido atrasado e alerta de conexão.
                  </p>
                </div>
...
${url}`)}catch{window.prompt('Copie o link exclusivo deste restaurante:',url)}}} className="px-3 py-1.5 rounded-xl text-xs font-bold border border-stone-300 bg-white text-stone-700 hover:bg-stone-50 flex items-center gap-1.5"><ExternalLink className="w-3.5 h-3.5"/> Copiar link exclusivo</button>
        ...
        {activeTab === 'config' && (
          <div className="max-w-3xl mx-auto space-y-4">
            ...
            {/* 🔒 Senhas de segurança (evolução v24_2) — Kanban individual
                (acesso restrito só à cozinha/balcão, sem o resto do painel) e
                limpeza de histórico (confirmação extra além da permissão da
                conta). Os dois campos abaixo só enviam algo quando
                preenchidos — deixar em branco mantém a senha já salva. */}
            <div className="border border-stone-200 rounded-2xl p-4 space-y-3">
              ...
              <div>
                <label className="block text-xs font-bold text-stone-700 mb-1">
                  Senha do Kanban individual{' '}
                  {localConfig.kanbanPasswordSet && (
                    <span className="text-emerald-600 font-normal">✓ já configurada</span>
                  )}
                </label>
                <p className="text-[11px] text-stone-500 mb-1.5">
                  Dá acesso só ao quadro de pedidos deste restaurante (sem cardápio, configurações
                  ou usuários) — pra cozinha/balcão, em <code>/kanban/{slug}</code>.
                </p>
                <input
                  type="password"
                  value={kanbanPasswordSettingInput}
                  onChange={(e) => setKanbanPasswordSettingInput(e.target.value)}
                  placeholder={localConfig.kanbanPasswordSet ? 'Digite pra trocar a senha atual' : 'Definir uma senha'}
                  className="w-full px-3 py-2 bg-stone-50 border border-stone-200 rounded-xl text-sm focus:ring-2 focus:ring-stone-800 focus:outline-none"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-stone-700 mb-1">
                  Senha de limpeza de histórico{' '}
                  {localConfig.historyClearPasswordSet && (
                    <span className="text-emerald-600 font-normal">✓ já configurada</span>
                  )}
                </label>
                <p className="text-[11px] text-stone-500 mb-1.5">
                  Se definida, será exigida (além da permissão da conta) toda vez que alguém tentar
                  apagar o histórico de pedidos entregues/cancelados deste restaurante.
                </p>
                <input
                  type="password"
                  value={historyClearPasswordSettingInput}
                  onChange={(e) => setHistoryClearPasswordSettingInput(e.target.value)}
                  placeholder={localConfig.historyClearPasswordSet ? 'Digite pra trocar a senha atual' : 'Definir uma senha'}
                  className="w-full px-3 py-2 bg-stone-50 border border-stone-200 rounded-xl text-sm focus:ring-2 focus:ring-stone-800 focus:outline-none"
                />
              </div>
              <p className="text-[10px] text-stone-400">
                As senhas só são salvas quando você clicar em "Salvar Configurações" no fim da página.
                Deixe em branco pra manter a senha atual sem alterar.
              </p>
            </div>
      ...
      {dispatchOrder && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-md rounded-3xl p-5 shadow-2xl border border-stone-200 space-y-4 max-h-[85vh] overflow-y-auto my-4">
      ...
      {showCancelAllModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-md rounded-3xl p-5 shadow-2xl border border-rose-300 space-y-4 max-h-[85vh] overflow-y-auto my-4">
            <div className="flex items-center justify-between border-b border-stone-100 pb-3">
      ...
      {/* Senha de limpeza de histórico própria deste restaurante (evolução v24_2) */}
      {showClearHistoryPasswordModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-sm rounded-3xl p-5 shadow-2xl border border-stone-200 space-y-4 max-h-[85vh] overflow-y-auto my-4">
            <div className="flex items-center justify-between border-b border-stone-100 pb-3">
              <h3 className="font-black text-base text-stone-900 flex items-center gap-2">
                <Trash2 className="w-5 h-5 text-red-500" /> Limpar histórico
              </h3>
              <button
                onClick={() => setShowClearHistoryPasswordModal(false)}
                className="p-1 text-stone-400 hover:text-stone-700"
              >
                <X className="w-5 h-5" />
              </button>
            ...
            <p className="text-xs text-stone-600">
              Este restaurante exige uma senha própria pra limpar o histórico de pedidos. Digite-a pra confirmar.
            </p>
            <input
              type="password"
              autoFocus
              value={clearHistoryPasswordInput}
              onChange={(e) => setClearHistoryPasswordInput(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && handleConfirmClearHistoryWithPassword()}
              placeholder="Senha de limpeza de histórico"
              className="w-full px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-xl text-sm focus:ring-2 focus:ring-red-500 focus:outline-none"
            />
            {clearHistoryError && <p className="text-xs text-red-600">{clearHistoryError}</p>}
            <div className="flex justify-end gap-2">
              ...
              <button
                onClick={handleConfirmClearHistoryWithPassword}
                disabled={!clearHistoryPasswordInput.trim() || clearHistoryLoading}
                className="px-4 py-2 bg-red-600 hover:bg-red-500 disabled:opacity-40 text-white rounded-xl font-bold text-xs"
              >
                {clearHistoryLoading ? 'Confirmando...' : 'Confirmar e limpar'}
              </button>
            </div>
          </div>
        </div>
