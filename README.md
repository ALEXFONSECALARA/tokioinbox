...
3. (Opcional, só na primeira vez) Gere alguns restaurantes de exemplo:
   `node tools/seed-restaurants.mjs`
...
| `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY` | Obrigatório em produção | Persistência real (sem isso, cai em `backend/storage__legacy-json__*.json`, que é efêmero no Render) |
...
- **Impressão:** fila persistente por restaurante/dispositivo, com estado
  ...
  `backend/print-bridge.mjs`) — o navegador nunca tem acesso direto e
