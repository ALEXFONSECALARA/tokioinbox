...
export async function listOrders(slug) {
  ...
  return orderRows.map((o) => ({ ...orderRowToApi(o, itemsByOrder.get(o.id)), restaurantSlug: slug }));
}
...
export async function getOrder(slug, id) {
  ...
  const { data: itemRows, error: itemErr } = await supabase
    ...
    .eq('order_id', id);
  if (itemErr) throw itemErr;
  return { ...orderRowToApi(orderRow, itemRows), restaurantSlug: slug };
}
...
export async function clearFinishedOrders(slug) {
  ...
  const { data: ids, error: selectErr } = await supabase.from('orders').select('id').eq('restaurant_id', restaurantId).in('status',['entregue','cancelado']);
