...
app.post('/api/:slug/orders', async (req, res) => {
  ...
  try {
    ...
    const schemaProblem = ['42P01', '42703', '42883', '23502', 'PGRST202', 'PGRST204'].includes(code) ||
...
app.delete('/api/:slug/orders/history', requireAdmin, requireOwnRestaurant, async (req,res)=>{ const {slug}=req.params; if(!hasPermission(req,'gerenciar_historico')&&!req.adminUser?.isMaster)return res.status(403).json({error:'Você não tem permissão para excluir histórico.',code:'HISTORY_DELETE_FORBIDDEN',requestId:req.requestId}); try{const result=await db.clearFinishedOrders(slug);if(!result?.verified){return res.status(500).json({error:'A exclusão foi solicitada, mas não foi possível confirmá-la no banco.',code:'HISTORY_DELETE_NOT_CONFIRMED',requestId:req.requestId});}broadcastOrderEvent(slug,{type:'history-cleared',updatedAt:new Date().toISOString()});res.json({ok:true,...result});}catch(err){logServerError(`Erro ao limpar histórico ${slug}`,err,{restaurantSlug:slug,details:{requestId:req.requestId}});res.status(500).json({error:'Não foi possível limpar o histórico.',code:err?.code||'HISTORY_CLEAR_FAILED',requestId:req.requestId});} });
...
app.get('/api/:slug/backups', requireAdmin, requireOwnRestaurant, async(req,res)=>{try{res.json({backups:await db.listRestaurantBackups(req.params.slug)})}catch(e){
  // Backups são uma ferramenta operacional, não podem bloquear o painel inteiro.
  // Em uma instalação que ainda não aplicou a migration da tabela, mostramos
  // lista vazia + aviso para o admin, em vez de um alert repetitivo.
  const code=String(e?.code||'');
  const missingTable=code==='42P01'||code==='PGRST205'||/restaurant_backups/i.test(String(e?.message||''));
  if(missingTable){return res.json({backups:[],warning:'A tabela de backups ainda não foi criada no Supabase. Aplique a migration de produção para habilitar backups persistentes.'});}
  logServerError('Erro ao listar backups',e,{restaurantSlug:req.params.slug,details:{requestId:req.requestId}});
  res.status(500).json({error:'Não foi possível listar backups.',code:code||'BACKUPS_LIST_FAILED',requestId:req.requestId})
}});
