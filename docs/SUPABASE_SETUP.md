...
1. Cole o conteúdo de `database/0001_init_schema.sql` → **Run**.
2. Cole o conteúdo de `database/0002_rls_policies.sql` → **Run**.
...
node backend/scripts/migrate-to-supabase.mjs
```

O script:
1. Faz backup de `backend/storage/legacy-json/` inteiro (pasta `backend/storage/legacy-json-backup-<data>/`).
...
**Nada em `backend/storage/legacy-json/` é apagado por este script.**
...
- **Uploads de imagem** (`/api/:slug/upload`) continuam salvando em disco
  local (`backend/storage/legacy-json/uploads/`), não no Supabase Storage. No plano free do
...
A migration `database/0016_media_library.sql` cria:
